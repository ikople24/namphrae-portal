...
// สรุปประจำวันเข้ากลุ่มได้ไหม, webhook (LINE_CHANNEL_SECRET) คือรับ groupId ตอน
...
function lineStatusMessage(status: LineGroupStatus): {
  ...
  if (status.push && status.webhook) {
    return {
      icon: 'check_circle',
      text: 'ตั้งค่า LINE ครบแล้ว — สรุปตารางงานประจำวัน (17:00) จะถูกส่งเข้ากลุ่มด้านล่าง',
    };
  }
  if (status.push) {
    return {
      icon: 'warning',
      text: 'ส่งสรุปประจำวันเข้ากลุ่มได้แล้ว แต่ยังเชิญบอทเข้ากลุ่มใหม่ไม่ได้ — ถ้าต้องตั้งกลุ่มใหม่หรือย้ายกลุ่ม ต้องแจ้งผู้ดูแลระบบก่อน (ผู้ดูแลระบบ: ตั้งค่า LINE_CHANNEL_SECRET ด้วย ไม่งั้น webhook รับ event จาก LINE ไม่ได้)',
    };
  }
  if (status.webhook) {
    return {
      icon: 'warning',
      text: 'รับข้อมูลกลุ่มจาก LINE ได้ แต่ยังส่งข้อความเข้ากลุ่มไม่ได้ — กลุ่มจะไม่ได้รับสรุปตารางงานประจำวัน (ผู้ดูแลระบบ: ตั้งค่า LINE_CHANNEL_ACCESS_TOKEN ด้วย)',
    };
  }
  return {
    icon: 'warning',
    text: 'ยังไม่ได้ตั้งค่าระบบแจ้งเตือน LINE — กลุ่มจะไม่ได้รับสรุปตารางงานประจำวัน (ผู้ดูแลระบบ: ตั้งค่า LINE_CHANNEL_ACCESS_TOKEN และ LINE_CHANNEL_SECRET ใน environment)',
  };
}
