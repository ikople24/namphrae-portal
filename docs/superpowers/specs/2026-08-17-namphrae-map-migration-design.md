...
| 5 | **`public/cmu_namphare.geojson` ขาดหมู่ 9** — ไฟล์มี 11 polygon แต่เป็นแค่ 10 หมู่ เพราะหมู่ 5 ถูกแบ่งเป็นสองรูป ยืนยันกับเลเยอร์ `zone-moobang` ของพอร์ทัลแล้วว่าตำบลมีหมู่ 1–11 ครบ ผลคือ `villageOf()` คืน `null` ให้ทุกพิกัดที่ตกในหมู่ 9 → เคสของหมู่ 9 จะหายจากสถิติรายหมู่เงียบ ๆ ไม่ใช่ error | รอบ 1 Task 11 | รอบ 2–3 ที่เริ่มใช้สถิติรายหมู่จริง — ต้องขอไฟล์ขอบเขตที่ครบจากต้นทาง ห้ามปั้นขอบเขตเอง |
| 6 | `ThaiDatePicker.tsx` ที่ยกมาติด lint rule `react-hooks/set-state-in-effect` ของ Next 16 ซึ่ง eslint ชุดเก่าของ namphrae-map ไม่มี — effect ซิงก์มุมมองปฏิทินตอนเปิดแผง ไม่ใช่ pattern derive-during-render ที่ rule ตั้งใจจับ ปิดด้วย eslint-disable พร้อมเหตุผลแทนการรื้อตรรกะ ตามกติกา "ยกมาโดยไม่แก้" (บรรทัดเดียวกันมี exhaustive-deps ปิดไว้จากต้นทางอยู่ก่อนแล้ว) | รอบ 2B Task 5 | รอบที่แตะ ThaiDatePicker จริง (รอบ 4 ที่ทะเบียนสาธารณสุขใช้มัน) — รื้อให้เป็น derived state หรือใช้ key แทนการ setState ใน effect |
