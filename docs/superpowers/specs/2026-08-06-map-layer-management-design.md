...
6. **มีข้อความไทยที่ encoding พังตอน export** — 57 ช่อง คือ `Id_Chanod` 55 แถว
   (`30427(เน€เธ?เธ?เน€เธ??…)`) กับ `land_type` 2 แถว

   > แก้จากตัวเลขเดิม 74 ที่บันทึกไว้ตอนสำรวจรอบแรก: การสแกนรอบนั้นใช้เงื่อนไข
   > หลวม ๆ (มี `เธ`/`เน`/`?` อยู่ในค่า) แล้วนับชื่อสถานที่ไทยที่ถูกต้องเป็นของเสียไป
   > 17 รายการ — `ยางเนิ้ง`, `สันพระเนตร`, `ราชดำเนิน`, `ทิพย์เนตร`, `ควนเนียง`,
   > `ล้านนาไพน์เนอรรี่โฮม` กฎที่ลงโค้ดจริงจึงบังคับว่า `เธ`/`เน` ต้องตามด้วยอักขระ
   > ที่ **ไม่ใช่** ตัวอักษรไทย ซึ่งเป็นรอยนิ้วมือของ mojibake ที่แท้จริง
